...
| **Библиотека** `/library` | собранные карточки: искать, править, отправлять в проект |
...
**Библиотека — исключение и единственное общее хранилище.** Собранная
...
(`items/<бренд>-<модель>.json`), отдельного индекса нет намеренно — он
немедленно разошёлся бы с файлами. Поиск идёт по всем текстовым полям
сразу: менеджер помнит товар то по бренду, то по отделке, то по обрывку
...
safe_fetch.py      запросы наружу с защитой от внутренних адресов
  ...
  расчёт и выгрузка
library.py         библиотека товаров: карточки в Vercel Blob, поиск
