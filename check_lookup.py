...
def check_final_block() -> tuple[int, int]:
    ...
    one = _pricing.for_position({"purchase": 1000, "qty": 1, "assembly": 1},
                                rates=None)
    checks += [
        # Подпись обязана описывать арифметику. «Безнал» рядом с «Суммой»
        # читался как доплата за способ оплаты, а это вершина лестницы
        # из четырёх наценок — к безналу относится лишь последний шаг.
        ("верхний уровень назван наценками, а не безналом",
         "Со всеми наценками, евро" in labels and "Безнал" not in labels),
        # Одно и то же число звалось внизу «Суммой», а в итоге компреда
        # «Суммой позиций» — разница искалась там, где её нет.
        ("итог позиций назван так же, как строка итога компреда",
         "Сумма позиций, евро" in labels
         and "Сумма позиций" in soup_lv.get_text(" ", strip=True)),
        ("обе величины равны по виду", 
         soup_lv.select_one("#t_finserv").get("class") in (None, [])),
        ("лестница наценок совпадает с книгой: каждая от предыдущей",
