...
Растаможка стоит в AJ — единственной свободной колонке формы (проверено
по `0000-Offer-…-AUR-FORM.xlsx`: занята в нуле строк). Вставлять её после
транспорта нельзя: сдвинулись бы AD..AI вместе со ставками $AF$1..$AI$1,
и строка, вставленная в готовую книгу, считала бы по чужим ячейкам.
Блок AK..AQ («× количество») трогать не нужно — растаможка входит в AD,
а AP считает именно AD.
...
def pricing_row(fields: dict, row: int) -> list[str]:
    """Скрытый расчёт T..AJ: то, что в книге считает цену.
    ...
    return [
        ...
        f"=Y{r}+Z{r}+AA{r}+AB{r}+AC{r}+AJ{r}",        # AD СУММА
        ...
        # РАСТАМОЖКА: только число, введённое руками. Ставки у неё нет —
        # она зависит от кода ТН ВЭД и партии, а не от объёма и цены.
        (float(fields["customs"]) if fields.get("customs") not in (None, "")
         else ""),                                    # AJ РАСТАМОЖКА
    ]
