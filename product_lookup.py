...
def parse_dims(raw: str, type_ru: str = "") -> tuple[float | None, float | None, float | None, bool]:
    ...
    # Диапазон по оси схлопываем в НАИБОЛЬШЕЕ. Правило одно на все оси и
    # названо вслух: по объёму считается перевозка, и занижать её нельзя.
    # Раньше диаметр и высота брали наибольшее, а длина с глубиной — то,
    # что написано первым: у раздвижного стола «160/220x90x75Н» выходило
    # 2,3 м³, а у «220/160x90x75Н» — 1,7, то есть 300 € разницы решал
    # порядок написания на сайте фабрики.
    #
    # Косая черта БЕЗ пробелов — диапазон: «36/92», «D25/31», «160/220».
    # С пробелами — разделитель осей: у VENICEM «125 cm / Ø 25 cm / 31 cm»
    # это три разных размера, и схлопывать их нельзя.
    def _widest(match):
        return f"{max(float(x) for x in re.findall(num, match.group(0))):g}"

    s = re.sub(rf"{num}(?:/{num})+", _widest, s)
